...
def logscreen():
    ...
    cmp1 = cpm
    cpm = 0
    utils.set_title(f"SupariMC by Ripxtar | Checked: {checked}\{len(Combos)}  -  Hits: {hits}  -  Bad: {bad}  -  2FA: {twofa}  -  SFA: {sfa}  -  MFA: {mfa}  -  Valid Mail: {vm}  -  Cpm: {cmp1*60}  -  Retries: {retries}  -  Errors: {errors}")
    time.sleep(1)
...
def Main():
    ...
    utils.set_title("SupariMC by Ripxtar")
    os.system('cls')
    try:
        loadconfig()
    except:
        print(Fore.RED+"There was an error loading the config. Perhaps you're using an older config? If so please delete the old config and reopen SupariMC.")
        input()
        exit()
    print(logo)
    try:
